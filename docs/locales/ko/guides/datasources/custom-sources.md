# Custom Data 소스 Development

실무 운영 가이드에서 Truthound을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.

## 개요

실무 운영 가이드에서 Truthound, `DataSourceProtocol`, `BaseDataSource`, `AsyncBaseDataSource`, Custom, DataSourceProtocol, BaseDataSource, AsyncBaseDataSource을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
...
실무 운영 가이드에서 Capabilities을(를) 다루는 항목입니다:
...
## Creating an Async Data 소스

실무 운영 가이드에서 I/O-bound을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
...
### Step 2: Implement the Async Data 소스
...
실무 운영 가이드에서 SQL, `BaseSQLDataSource`, BaseSQLDataSource을(를) 다루는 항목입니다:
...
1. 실무 운영 가이드에서 `schema`, `get_execution_engine`, `sample`, `to_polars_lazyframe`, Implement을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
2. 실무 운영 가이드에서 `_cached_schema`, Cache을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
3. 실무 운영 가이드에서 `max_rows`, `max_memory_mb`, Respect, Check을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
4. 실무 운영 가이드에서 `capabilities`, Declare, Override을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
5. 실무 운영 가이드에서 `__enter__`, `__exit__`, Support, Implement을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
6. 실무 운영 가이드에서 Leverage을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
7. 실무 운영 가이드에서 Handle을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.
8. 실무 운영 가이드에서 Document, Add을(를) 기준으로 데이터 품질 검증, 워크플로우 자동화, 결과 해석 방법을 설명합니다.

...
개인정보(PII) 패턴을 탐지합니다. 문자열 컬럼별 첫 1,000개 비결측값을 검사하고, 전체 비결측 건수로 발견 건수를 추정합니다. 전 행 검사를 보장하지 않습니다. 전체 프레임 대신 스칼라 집계와 지연 실행된 컬럼별 제한 표본을 수집합니다. PyArrow가 설치되어 있으면 로컬 Parquet는 1,024행 단일 스레드 배치로 읽고 동일 판정 함수를 사용합니다. PyArrow가 없으면 지연 실행 경로를 사용합니다. 파일 읽기 버퍼나 매우 큰 개별 값에는 추가 메모리가 필요할 수 있으며 모든 환경의 메모리 상한을 보장하지 않습니다.

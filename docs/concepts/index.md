...
boundary-level Truthound overview in one site, but those layers
...
- how private Workflow engine primitives let Truthound and Truthound
...
| **Truthound AI** | Review layer | prompt-to-proposal compilation, canonical run analysis, approval logs, controlled suite apply, provider/redaction contracts | direct mutation of core state before review, Workflow runtime ownership |
| **Truthound Orchestration** | Execution integration layer | Airflow, Dagster, Prefect, dbt, Mage, and Kestra adapters plus a shared runtime contract | the kernel result model itself, Workflow business state |
| **Truthound** | Repository console / business-state layer | Workflow, branch, snapshot, merge request, release, rollback, evidence, audit, access, and operator workflows | validation execution semantics, planner/runtime logic, host-native execution projection |
...
| **First-party external layers** | `truthound-orchestration`, Truthound | separate operational layers; orchestration remains mirrored here, while Workflow is represented through curated overview pages rather than a full mirrored manual |
...
- Move to **Workflow** if you need a dataset repository console with branches,

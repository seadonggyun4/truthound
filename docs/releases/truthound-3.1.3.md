...
completed Truthound Depot product direction.

## What's New

- public product-line wording now identifies Truthound Depot as the first-party
...
- Core is documented as the data-plane primitive owner for validation
  ...
  projections
- Depot is documented as the business-state owner for branch, merge request,
...
- Data Docs dashboard wording now distinguishes the local dashboard UI from the
  Depot repository console product
...
- no core workspace, checkpoint, reporter, or AI review contract is broken by
  this patch release
- the private Depot engine primitive namespace remains private and does not
  introduce a public `truthound.datasets` or `truthound.depot` API

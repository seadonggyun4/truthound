"""Shared redaction helpers for Truthound payloads and artifacts."""
...
class SummaryOnlyRedactor:
    ...
    def inspect_text(self, text: str, *, path: str) -> list[RedactionViolation]:
        ...
        if findings:
            pii_types = ", ".join(
                sorted({str(item.get("pii_type", "unknown")) for item in findings})
            )
            return [
                RedactionViolation(
                    path=path,
        ...
        if (
            self._path_looks_forbidden(path)
            or any(marker in lowered for marker in FORBIDDEN_CONTEXT_MARKERS)
    ...
    def _path_looks_forbidden(self, path: str) -> bool:
        lowered = path.lower()
        return any(
            f".{marker}" in lowered or lowered.endswith(marker)
